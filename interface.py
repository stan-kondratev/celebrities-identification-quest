...
if 'score_list' not in st.session_state:
    st.session_state['score_list'] = [(0,0)]
...
with col1:
    ...
    with col1_1:
        if st.button("Guess", use_container_width=True):
            ...
            st.session_state["score_list"].append([a,score[1]])
...
with col2:
    ...
    st.text(" ")
    st.text(" ")
    df_score = pd.DataFrame(st.session_state["score_list"], columns=["Name","Score"])
    st.dataframe(df_score.iloc[1:].sort_values(by="Score", ascending=False),use_container_width=True,column_config={0:"Attempt",1:"Name",2:"Score"})
